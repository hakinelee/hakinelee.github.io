...
本篇文章主要介绍文件操作与隐写、图片隐写术、压缩文件处理、流量取证技术等。

## 一、文件操作

### 分析下载的文件：文件类型的识别

对于没有后缀名的文件类型识别。（原理：每个文件都会有相应的结构，根据文件头识别不同的文件）

1. 手工分析(windows系统)：`WinHex`/`010 Editor`/`notepad++`

   > `notepad++`需要搭配插件`hex editora`

2. 工具分析Linux：当文件出现无法打开的情况，需要查看文件头，如果损坏或者缺失需要进行相应补充。
...
**方法二**：windows下通过 `winhex` 查看文件头字段识别文件类型。最好使用Notepad++，安装插件HEX-Editor进行查看。

文件头残缺情况： 使用十六进制编辑器 `010 editor` 编辑器进行编辑。

![常见的头文件类型](../../assets/img/misc/misc-1.png)
...
通常将要识别的文件拖入 `winhex` 中，查找具有关键字或明显与文件内容不和谐的部分，通常优先观察文件头部和尾部，搜索 `flag` 或 `key` 等关键字，最后拖动滚轮寻找。

#### `Notepad++`

使用 `notepad++` 打开文件，查看文件头尾是否含有关键字的字符串，搜索 `flag` 或 `key` 等关键字，最后拖动滚轮寻找。另外通过安装插件 `HEX-Editor` 可以实现 `winhex` 的功能。
...
例: Stegdetect提示jphide加密时，可以用Jphs工具进行解密，打开 `jphswin.exe`，使用 `open jpeg` 打开图片，点击seek,输入密码和确认密码，在弹出文件框中选择要保存的解密文件位置即可，结果保存成txt文件。
...
使用场景: `Stegdetect` 识别出来是F5加密的图片或题目提示是F5加密的图片
   > 进入 `F5-steganography_ F5` 目录，将图片文件拷贝至该目录下，从 CMD 进入该目录。

格式: `Java Exrtact` 要解密的文件名-p密码
